...
BOOL CThreadsSynchroDlg::OnInitDialog()
{
	...
	else
	{
		...
		SetWindowText(_T("Принимаю"));
		SetDlgItemTextW(IDC_NUMER, _T("Вторичный"));
		active = false;
...
void CThreadsSynchroDlg::OnClose()
{
	...
	UnmapViewOfFile(lp_BaseAddress);		// отключение от текущего процесса объекта файлового отображения
	CloseHandle(hObject);
	CDialogEx::OnClose();
}
